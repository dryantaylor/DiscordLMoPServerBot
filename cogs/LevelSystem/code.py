...
class LevelSystem(commands.Cog):
    ...
    async def on_member_join(self, member):
        if member.id not in self.exp_table[member.guild.id].keys():
            self.exp_table[str(member.guild.id)][str(member.id)] = [0,0,0]
    ...
    async def on_guild_join(self,guild):
        print("hello aaaa")
        self.exp_table[str(guild.id)] = {}
        for member in guild.members:
            if member.id not in self.exp_table[str(guild.id)].keys() and not member.bot:
                self.exp_table[str(guild.id)][str(member.id)] = [0, 0, 0]
    ...
    async def on_message(self,message):
        if not message.author.bot:
            ...
            try:
                ...
                if currTime - self.exp_table[guild_id][str(message.author.id)][0] > minTimeBetween:
                    exp = self.exp_table[guild_id][str(message.author.id)][1]
                    level = self.exp_table[guild_id][str(message.author.id)][2]
                    self.exp_table[guild_id][str(message.author.id)][0] = currTime #time since last exp gain
                    ...
                    self.exp_table[guild_id][str(message.author.id)][1] = exp
                    self.exp_table[guild_id][str(message.author.id)][2] = level
                    with open(self.fileLoc,"w") as file:
                        json.dump(self.exp_table,file)
    ...
    async def exp(self,ctx):
        user = self.exp_table[str(ctx.guild.id)][str(ctx.author.id)]
        exp_to_next_level = self.calc_exp_to_next_level(user[2])

        exp_bar = self.calc_exp_bar_draw(user)
        rank,n_users = self.calc_user_rank(str(ctx.guild.id),str(ctx.author.id))
        # ◕  ◔  ⬤  ○ ◐
    ...
    def calc_user_rank(self,guild_id: str,user_id: str):

        user_level = int(self.exp_table[guild_id][user_id][2])
        user_exp = int(self.exp_table[guild_id][user_id][1])
        rank = 1
        n_users = 0
        user_id = user_id
        for user in self.exp_table[guild_id].items():
            if user[0] != user_id:
                pass
